...
local PORTALS = {
    ...
    { Portal = "SnowIsland",   MobName = "FrostRogue",        FarmTime = 1.5,   IsBossEntry = true  },
    { Portal = "Shibuya",      MobName = "Sorcerer",          FarmTime = 1.5     },
    { Portal = "Hollow",       MobName = "Hollow",            FarmTime = 1.5     },
    { Portal = "Shinjuku",     MobName = "Curse",             FarmTime = 1.5    },
    { Portal = "Shinjuku",     MobName = "Strong",            FarmTime = 1.5     },
    { Portal = "Slime",        MobName = "Slime",             FarmTime = 1.5     },
    { Portal = "Academy",      MobName = "AcademyTeacher",    FarmTime = 1.5   },
    { Portal = "Judgement",    MobName = "Swordsman",         FarmTime = 1.5     },
    { Portal = "SoulDominion", MobName = "Quincy",            FarmTime = 3.5   },
    { Portal = "Ninja",        MobName = "Ninja",             FarmTime = 1.5     },
    { Portal = "Lawless",      MobName = "ArenaFighter",      FarmTime = 1.5     },
...
local HakiRmt      = RemoteEvents:WaitForChild("HakiRemote", 5)
local ObsRmt       = RemoteEvents:WaitForChild("ObservationHakiRemote", 5)
...
local farmFloat=nil
local scriptReady=false  -- กัน DoTeleport ไม่ให้ทำงานก่อน UI โหลดเสร็จ

-- ประกาศก่อน DoTeleport เพราะ DoTeleport ใช้
...
local function DoTeleport()
    ...
    if not scriptReady then return end  -- ไม่วาปถ้า script ยังไม่พร้อม
    tping=true; lastTP=tick()

    -- ถ้าเป็น Boss entry → หา SpawnPoint ใน Island folder โดยตรง ไม่ผ่าน portal
    if CurrentEntryIsBoss() then
        RefreshChar()
        if HRP then
            -- map boss portal → folder ใน workspace
            local BOSS_ISLAND_MAP = {
                Sailor  = "SailorIsland",
                Shibuya = "ShibuyaIsland",
                Hollow  = "HollowIsland",
                Ninja   = "NinjaIsland",
            }
            local folderName = BOSS_ISLAND_MAP[CFG.Portal] or (CFG.Portal.."Island")
            local folder = workspace:FindFirstChild(folderName)
            if not folder then
                -- fallback หาชื่อคล้ายกัน
                for _, v in ipairs(workspace:GetChildren()) do
                    if v.Name:lower():find(CFG.Portal:lower(), 1, true) then
                        folder = v; break
                    end
                end
            end
            ...
            if folder then
                -- หา SpawnPoint/SpawnLocation ใน folder
                local spawnPart = nil
                ...
                if spawnPart then
                    ...
                    print("[Boss TP] วาปไป SpawnPoint: "..spawnPart.Name)
                    tping=false; return
                end
            end

            -- fallback: ค้นหา boss ที่ spawn อยู่แล้วใน workspace
            ...
            for _, m in ipairs(workspace:GetDescendants()) do
                if m:IsA("Model") and m ~= Char then
                    ...
                    if h and h.Health > 0 and r then
                        if m.Name:lower():find(mobLow, 1, true) then
                            HRP.CFrame = CFrame.new(r.Position + Vector3.new(0, 5, 3))
                            print("[Boss TP] เจอ boss → วาปตรง")
                            tping=false; return
                        end
                    end
                end
...
Options.FarmEnabled:OnChanged(function()
    ...
    if CFG.Enabled then
        ...
        -- คืนสภาพตัวละครให้ปกติ
        frozenCF=nil; freezeUntil=0
        RefreshChar()
        if HRP then
...
Tabs.Main:AddToggle("AutoRespawn", {
    Title="Auto Respawn", Description="รี spawn ทุก N วิ เพื่อแก้อาการค้าง", Default=true,
})
Options.AutoRespawn:OnChanged(function() CFG.AutoRespawn=Options.AutoRespawn.Value end)

Tabs.Main:AddSlider("RespawnInterval", {
    Title="Respawn ทุกกี่วิ", Description="default 300 = 5นาที",
    Default=300, Min=60, Max=600, Rounding=0,
    Callback=function(v) CFG.RespawnInterval=v end,
})

local StatusPara = Tabs.Main:AddParagraph({ Title="Live Status", Content="Status: OFF" })

-- ── Haki Tab ──────────────────────────────────────────────
Tabs.Haki:AddParagraph({
    Title   = "Auto Haki",
    Content = "เปิดไว้ตลอด — กด G/H ซ้ำอัตโนมัติทุกๆ 1 วิ\nพอตายจะเปิดใหม่อัตโนมัติหลัง spawn",
})

Tabs.Haki:AddToggle("BusoOn", {
    Title       = "Armament Haki (Buso)",
    Description = "กด G ซ้ำตลอดเวลา",
    Default     = false,
})

Tabs.Haki:AddToggle("ObsOn", {
    Title       = "Observation Haki (Obs)",
    Description = "กด H ซ้ำตลอดเวลา",
    Default     = false,
})
...
Tabs.Misc:AddParagraph({
    Title   = "Remove Map",
    Content = "ลบแมพทั้งหมด เหลือแค่ฟ้า + mob + ตัวละคร\nโล่งลื่นสุดๆ เหมือนในภาพ",
})

Tabs.Misc:AddButton({
    Title       = "🗑️ REMOVE MAP",
    Description = "ลบ Island/Part/Effect ทั้งแมพ เหลือแค่ตัวละครกับ mob",
    Callback    = function()
        local removed = 0

        -- ปิด Lighting ทั้งหมด
        pcall(function()
        ...
        -- เก็บชื่อ character ของ player ทุกคน
        local keepChar = {}
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Character then keepChar[p.Character.Name] = true end
        end

        -- ลบทุกอย่างใน workspace ยกเว้น Terrain, Camera, Character, Model ที่มี Humanoid
        for _, obj in ipairs(workspace:GetChildren()) do
            ...
            if obj:IsA("Model") then
                -- ข้าม model ที่มี Humanoid (mob/npc/player)
...
Tabs.Misc:AddButton({
    Title="🚀 FPS Boost", Description="ลบ Decal/Particle/Shadow/Atmosphere เท่านั้น",
...
-- โหลด config หลังสร้าง UI ครบแล้ว
SaveManager:LoadAutoloadConfig()

-- บังคับปิด Farm เสมอตอนรัน script ใหม่
task.defer(function()
    Options.FarmEnabled:SetValue(false)
    CFG.Enabled = false

    -- sync ค่าอื่นๆ จาก config ที่โหลดมา (ยกเว้น Farm)
    ...
    if Options.PortalSelect and Options.PortalSelect.Value then
        ...
        for i,v in ipairs(PORTALS) do
            if (v.Portal.." - "..v.MobName)==val then
                CFG.Portal=v.Portal; CFG.MobName=v.MobName; portalIndex=i
                break
            end
        end
    end

    print("[Config] โหลดเสร็จ — Farm ปิดอยู่ กด Toggle เพื่อเริ่ม")
    scriptReady = true  -- อนุญาตให้ DoTeleport ทำงานได้แล้ว
end)
...
local function FindNearestMob()
    ...
    local mobLow=CFG.MobName:lower(); local wantBoss=CurrentEntryIsBoss()
...
-- ══════════════════════════════════════════════════════
-- TPToTarget — แก้ค้างบนหัว mob
-- เพิ่ม task.wait เล็กน้อย + detect stuck + offset สุ่ม
-- ══════════════════════════════════════════════════════
local lastTPPos   = Vector3.zero
local lastTPTime  = 0
local stuckCount  = 0
...
local function TPToTarget(root)
    ...
    -- detect stuck: ถ้าอยู่แทบไม่ขยับนานกว่า 1.5 วิ นับว่า stuck
    local moved = (HRP.Position - lastTPPos).Magnitude
    if moved < 0.5 and now - lastTPTime > 1.5 then
        stuckCount += 1
    else
        stuckCount = 0
    end
    ...
    -- ถ้า stuck → เพิ่ม offset สุ่มเพื่อหลุดออก
    local offsetY = 2  -- ความสูงจาก mob (ต่ำลง เพราะอยู่ข้างหลัง)
    local offsetX = 0
    local offsetZ = 3  -- ระยะข้างหลัง mob
    if stuckCount >= 3 then
        offsetY = math.random(1, 4)
        offsetX = math.random(-3, 3)
        offsetZ = math.random(3, 6)
        stuckCount = 0
        print("[StuckFix] หลุดออกจาก mob")
    end

    local dest = CFrame.new(
        root.Position + Vector3.new(offsetX, offsetY, offsetZ)
    ) * CFrame.new(0, 0, 3)  -- อยู่ข้างหลัง mob 3 studs
    ...
    -- delay เล็กน้อยก่อนยิง CombatRmt เพื่อให้ position settle
...
local function OnIslandReal()
    ...
    for _,m in ipairs(npcs:GetDescendants()) do
        if m:IsA("Model") and m.Name:lower():find(mobLow,1,true) then
            ...
            if root and (HRP.Position-root.Position).Magnitude<600 then return true end
        end
    end
    return false
end
...
task.spawn(function()
    while true do
        ...
        if CFG.SkillX and AbilityRmt and now-lastSkillX>=CFG.SkillCoolX then
            lastSkillX=now; pcall(function() AbilityRmt:FireServer(2) end)
        end
...
task.spawn(function()
    while true do
        ...
        print("[AutoRespawn] รี spawn...")
        Fluent:Notify({ Title="Auto Respawn", Content="รี spawn แก้อาการค้าง 🔄", Duration=3 })
        -- ไม่ปิด CFG.Enabled เพื่อไม่ให้ UI toggle ดูเหมือนปิด
        frozenCF=nil; freezeUntil=0
        RefreshChar()
        if HRP then
        ...
        -- farm ยังเปิดอยู่ ไม่ต้อง set ใหม่
...
-- ══════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════
-- AUTO HAKI — กดครั้งเดียว ไม่กดซ้ำ จนกว่าจะตาย
-- ══════════════════════════════════════════════════════
...
local hakiPressed = false  -- กดไปแล้วหรือยัง

-- กดครั้งแรกตอนเปิด toggle
...
-- พอตาย → รอ spawn ใหม่ → กดเปิดอีกครั้ง
LP.CharacterAdded:Connect(function()
    task.wait(2.5)
    RefreshChar()
